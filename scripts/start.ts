...
switch (process.platform) {
	case "win32": {
		const gitExist = fs.existsSync(
			path.join(LOCAL_GIT_DIRECTORY, "mingw64/bin", "git.exe"),
		);
		if (gitExist) {
			console.log("> Found git.exe");
		} else {
			throw new Error(
				"'git.exe' is missing, this can be caused by node_modules issues.",
			);
		}
		break;
	}
	...
	case "darwin": {
		const gitExist = fs.existsSync(
			path.join(LOCAL_GIT_DIRECTORY, "bin", "git"),
		);
		if (gitExist) {
			console.log("> Found git executable");
		} else {
			throw new Error(
				"'git' is missing, this can be caused by node_modules issues.",
			);
		}
		break;
	}
...
const viewerProcess = attachStdio(
	exec(
		`npx lerna exec --scope @noodl/noodl-viewer-react -- npm run ${viewerScript}`,
		processOptions,
	),
	{
		prefix: "Viewer",
...
const cloudRuntimeProcess = attachStdio(
	exec(
		`npx lerna exec --scope @noodl/cloud-runtime -- npm run ${viewerScript}`,
		processOptions,
	),
	{
		prefix: "Cloud",
...
const editorProcess = attachStdio(
	exec("npx lerna exec --scope roodl-editor -- npm run start", processOptions),
	{
		prefix: "Editor",
