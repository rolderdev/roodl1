...
function parseRangeHeader(range, length) {
	...
	const end = Number.parseInt(partialend, 10);
...
function startServer(
					...
					(injected) => {
						projectGetSettings((settings) => {
							...
							injected = injected.replace(
								"{{#title#}}",
								settings.htmlTitle || "Roodl Viewer",
							);
							injected = injected.replace(
								"{{#customHeadCode#}}",
								settings.headCode || "",
	...
	function handleRequest(request, response) {
		...
		if (
			path === "/external/viewer/index.html" ||
			path.endsWith("viewer/index.htmlnull")
		) {
		...
		if (
			path.includes("/noodl_modules/") === false &&
			(path.endsWith("index.html") || path.includes(".") === false)
		) {
			serveIndexFile(appPath + "/src/external/viewer/index.html", response);
			return;
		}
		...
		(() => {
			...
			ws.on("message", (message) => {
				...
				// If this is a request to a service, pass it along to the service
					...
					if (request.target) {
						for (var i = 0; i < connectedSockets.length; i++)
							if (connectedSockets[i].clientId === request.target)
