...
const {
	startCloudFunctionServer,
	closeRuntimeWhenWindowCloses,
} = require("./src/cloud-function-server");
...
`);
	...
	function makeEditorAPIRequest(api, args, callback) {
		...
		if (win && win.webContents && !win.webContents.isDestroyed()) {
			win.webContents.send("editor-api-request", {
				api: api,
				token: t,
				args: args,
			});
	...
	ipcMain.on("editor-api-response", (event, args) => {
	...
	function createWindow() {
		...
		process.env.noodlURI &&
			win.webContents.send("open-noodl-uri", process.env.noodlURI);
	...
	function openFloatingWindow(options) {
		if (!floatingWindows[options.id])
			floatingWindows[options.id] = new FloatingWindow();
	...
	const buildNumber = JSON.parse(
		fs.readFileSync(appPath + "/package.json"),
	).buildNumber;
	...
	let submenu = [
		{
			...
			click: () => {
				require("about-window").default({
					...
					copyright: "Copyright (c) 2024 Roolder LLC",
	...
	function setupAskForMediaAccessIpc() {
		...
		ipcMain.on("request-media-access", (event, mediaTypes) => {
	...
	function setupFloatingWindowIpc() {
		ipcMain.on("floating-window-close", (event, options) => {
			closeFloatingWindow(options);
		});
		ipcMain.on("floating-window-open", (event, options) => {
	...
	function setupMainWindowControlIpc() {
		ipcMain.on("main-window-resize", (event, options) => {
