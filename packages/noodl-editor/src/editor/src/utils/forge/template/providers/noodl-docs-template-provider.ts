...
export class NoodlDocsTemplateProvider implements ITemplateProvider {
	get name(): string {
		return this.getDocsEndpoint() || "https://docs.rolder.ru";
	}
	...
	async list(_options: TemplateListFilter): Promise<readonly TemplateItem[]> {
		...
		templates.forEach((t) => {
			...
			if (
				t.cloudServicesTemplateURL !== undefined &&
				!t.cloudServicesTemplateURL.startsWith("http")
			) {
				t.cloudServicesTemplateURL =
					endpoint + "/projecttemplates/" + t.cloudServicesTemplateURL;
			}
		});
	...
	private async makeRequest(endpoint: string) {
		const url =
			endpoint + "/projecttemplates/index.json" + "?" + new Date().getTime();
