...
function EditorDocument() {
	...
	useEffect(() => {
		...
		nodeGraph.on(
			...
			({ model }) => EditorSettings.instance.setMerge(ProjectModel.instance.id, { selectedComponentName: model.fullName }),
			eventGroup
		)
	...
	useEffect(() => {
		...
		setPreviewMode(Boolean(settings.previewMode))
	}, [nodeGraph])
